...
const team = [
  ...
  },
  {
    ...
    bio: 'Especialista en el trato con clientes y en la gestión de operaciones de compraventa. Josep aporta al equipo experiencia, compromiso y una visión clara del mercado local.',
  },
];
    ...
    <div className="pt-20 bg-[var(--cream)]">
      ...
      <section className="relative h-72 md:h-96 overflow-hidden">
        ...
        <div className="absolute inset-0 flex items-center justify-center text-center px-6">
          <div>
            <p className="text-[var(--gold)] text-sm font-semibold tracking-[0.3em] uppercase mb-3">{t('badge')}</p>
      ...
      <section className="py-20 px-6">
        <div className="max-w-4xl mx-auto text-center">
          <h2 className="text-[var(--navy)] font-bold text-3xl md:text-4xl mb-6 gold-line gold-line-center font-playfair">
            {t('subtitle')}
          </h2>
        </div>
      </section>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
            {[
              ...
              <div key={label} className="text-center">
                <p className="text-[var(--gold)] font-bold text-4xl font-playfair">
                  {value}
                </p>
      ...
      <section className="py-20 px-6 bg-white">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-12">
            <p className="text-[var(--gold)] text-sm font-semibold tracking-[0.3em] uppercase mb-2">{t('team.badge')}</p>
            <h2 className="text-[var(--navy)] font-bold text-4xl gold-line gold-line-center font-playfair">
              {t('team.title')}
            </h2>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 max-w-5xl mx-auto">
            {team.map(({ name, role, img, bio }) => (
              <div key={name} className="text-center group">
                <div className="relative w-40 h-40 rounded-full overflow-hidden mx-auto mb-4 ring-4 ring-[var(--cream)] group-hover:ring-[var(--gold)] transition-all duration-300">
                  <Image src={img} alt={name} fill className="object-cover group-hover:scale-105 transition-transform duration-500" />
                </div>
                <h3 className="font-bold text-[var(--navy)] text-lg font-playfair">{name}</h3>
                <p className="text-[var(--gold)] text-sm font-medium mb-3">{role}</p>
      ...
      <section className="py-20 px-6 bg-[var(--cream)]">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-12">
            <p className="text-[var(--gold)] text-sm font-semibold tracking-[0.3em] uppercase mb-2">{t('values.badge')}</p>
            <h2 className="text-[var(--navy)] font-bold text-4xl gold-line gold-line-center font-playfair">
              {t('values.title')}
            </h2>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {valueKeys.map((key) => (
              <div key={key} className="bg-white rounded-2xl p-6 shadow-sm hover:shadow-md transition-shadow border border-gray-100">
                <div className="w-12 h-12 rounded-xl gold-gradient flex items-center justify-center text-[var(--navy)] mb-4">
                  {valueIcons[key]}
                </div>
                <h3 className="text-[var(--navy)] font-bold text-lg mb-2">
                  {t(`values.${key}.title`)}
                </h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            {Object.entries(zoneDescs).map(([name, desc]) => (
              <Link
                ...
                className="group flex items-start gap-4 p-5 rounded-xl border border-gray-100 hover:border-[var(--gold)] hover:bg-[var(--cream)] transition-all"
              >
                <div className="w-2 h-2 rounded-full bg-[var(--gold)] mt-2 shrink-0 group-hover:scale-150 transition-transform" />
                <div>
                  <p className="font-semibold text-[var(--navy)] mb-1 group-hover:text-[var(--gold)] transition-colors">{name}</p>
      ...
      <section className="py-16 px-6 gold-gradient">
        <div className="max-w-2xl mx-auto text-center">
          <h2 className="text-[var(--navy)] font-bold text-3xl mb-4 font-playfair">
            {t('cta.title')}
          </h2>
